...
def generate_transfer_file(month, year):
    ...
    content += make_line("   <ZRVON type=\"jahrmonat\">{0:04d}-{1:02d}</ZRVON>".format(int(year), int(month)))
    content += make_line("   <ZRBIS type=\"jahrmonat\">{0:04d}-{1:02d}</ZRBIS>".format(int(year), int(month)))
